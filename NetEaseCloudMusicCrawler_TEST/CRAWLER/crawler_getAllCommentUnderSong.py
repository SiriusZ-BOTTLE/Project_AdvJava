...
import CRAWLER.crawler_commentsUnderSong as tool
from CRAWLER.crawler_getAllSongDetail import get_all_song_link_from_song_records_file
...
def main():
    ...
    # 逐个遍历歌曲链接, 爬取歌曲下评论详细信息
    count = 0  # 预先设定从哪里开始爬
    max_size = 1000  # 最终位置

    file_csv = open("list_comment_details.csv", "a", encoding='utf-8')
    ...
    while count < len(list_links_song) and count < max_size:
        try:
            ...
            comments = tool.get_comment_under_song(list_links_song[count], driver)
            # print(count, comments)  # 打印
            # 逐个写入到文件
